...
if __name__ == '__main__':
    ...
    while iterations < args.num_iterations:
        ...
        solutions = Solutions()
        solutions.search_space = args.search_space
        solutions.search_method = args.search_method
        solutions.num_iterations = args.num_iterations
        solutions.current_iteration = iterations
        solutions.best_reward = best_reward
        solutions.solution = dsl.parse_node_to_str(best_prog)
        solutions.task = args.task
        solutions.uuid = str(myuuid)
        solutions.crashable = env_args["crashable"]
        solutions.leaps_behaviour = env_args["leaps_behaviour"]
        solutions.seed = args.seed
        solutions.save()
        if best_reward == 1:
            break

...
class BaseSearch(ABC):
    ...
    def __init__(self, dsl: BaseDSL, env_cls: type[BaseEnvironment],
                 ...
                 latent_model_params_path: str = None, search_seed: int = 1, qtd_dicts: int = 1,
        ...
        self.dsl = dsl
        self.task_cls_name = task_cls_name
    ...
    def init_output(self):
        ...
        if self.qtd_dicts > 0:
            with open(os.path.join(self.task_output_folder, f'seed_{self.search_seed}_qtd_{self.qtd_dicts}.csv'), 'w') as f:
                f.write(','.join(fields))
                f.write('\n')
        else:            
    ...
    def save_best(self):
        ...
        if self.qtd_dicts > 0:
            with open(os.path.join(self.task_output_folder, f'seed_{self.search_seed}_qtd_{self.qtd_dicts}.csv'), 'a') as f:

...
class DictionaryControl:
    def __init__(self, dsl, seed, task):
        self.dsl = dsl
        #self.path_dic = 'datasets/dictionaries/randomDicTest'+str(seed)+'.txt'
        self.path_dic = 'datasets/dictionaries/dic'+task+'NoCrash/dict1.txt'
        self.dict_program = []
        self.env_generators = []
        self.dict_nodes = set()
        for i in range(32):
    ...
    def load_dictionary(self)-> None:   
        print('Dictionary used: ',self.path_dic)             
        with open(self.path_dic) as f:
...
class StochasticHillClimbingDictWithAnnealingParameter(BaseSearch):
    ...
    def init_search_vars(self):
        ...
        self.dicProgram = DictionaryControl(self.dsl, self.search_seed, self.task_cls_name)
    ...
    def search_iteration(self):
        ...
        for _ in range(self.k):
            ...
            while not accepted:
                ...
                #if self.diff < 0 or rand() < self.metropolis:
                    self.mutate_node(node_to_mutate)
                else:
                    self.mutate_node_by_dic(node_to_mutate)
        ...
        for prog in neighbors:
            ...
            if reward > self.current_reward:
                ...
                delta =  reward - self.current_reward          
                pre_p =   math.e **(-delta/self.temp) 
                self.p = min (0.8,pre_p)
                # difference between candidate and current point evaluation
                # self.diff = reward - self.current_reward
                # # calculate temperature for current epoch
                # t = self.temp / float(self.current_iteration + 1)
                # # calculate metropolis acceptance criterion
                # self.metropolis = exp(-self.diff / t)
                self.temp = self.temp * self.cool
                # print('Current diff ', str(self.diff))
                # print('Current metropolis ', str(self.metropolis))

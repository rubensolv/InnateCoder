...
class DictionaryControl:
    def __init__(self, dsl, seed, task):
        self.dsl = dsl
        self.path_dic = 'datasets/dictionaries/randomDictsCrashable/randomDicTest'+str(seed)+'.txt'
        #self.path_dic = 'datasets/dictionaries/dicLLM/dict'+str(seed)+'.txt'
        self.dict_program = []
        self.env_generators = []
        self.dict_nodes = set()
        for i in range(32):
    ...
    def clean_dict_by_behavior(self)-> None:     
        print('Starting cleanning process.... size ', str(len(self.dict_program)))                   
        ...
        print('Finished cleanning process....')                      
...
class StochasticHillClimbingLLMDict(BaseSearch):

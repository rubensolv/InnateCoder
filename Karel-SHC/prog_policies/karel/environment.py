...
class KarelEnvironment(BaseEnvironment):
    ...
    def to_image(self) -> np.ndarray:
        ...
        if len(assets) == 0:
            ...
            for f in files:
                assets[f] = np.array(Image.open(os.path.join('/home/rubens/pythonProjects/Karel-SHC/prog_policies/karel/assets', f'{f}.PNG')))
        _, rows, cols = self.state_shape
        img = np.ones((rows*grid_size, cols*grid_size))
        hero_r, hero_c, hero_d = self.get_hero_pos()
        for r in range(rows):
            for c in range(cols):
                if self.state[4][r][c] == 1:
                    asset = assets['wall']
                elif r == hero_r and c == hero_c:
                    if np.sum(self.state[6:, r, c ]) > 0:
                        asset = np.minimum(assets[f'agent_{hero_d}'], assets['marker'])
                    else:
                        asset = assets[f'agent_{hero_d}']
                elif np.sum(self.state[6:, r, c ]) > 0:
                    asset = assets['marker']
                else:
                    asset = assets['blank']

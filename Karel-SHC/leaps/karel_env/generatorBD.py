...
def generator(config):
    ...
    while(1):
        ...
        solution = Solutions_Leaps()
        solution.search_space = 'Teste'#'Crashable'
        solution.search_method = 'Leaps Random Generator'
        solution.num_iterations= num_total
        solution.task = karel_world.task_definition
        solution.solution = random_code
        solution.best_reward = 0.0
        solution.current_iteration = count        
        solution.id_task = id
        solution.state_task = s
        solution.seed = config.seed
        solution.save()
        # progress bar
        count += 1
...
def main():
    ...
    parser.add_argument('--num_train', type=int, default=100, help='num train')
    parser.add_argument('--num_test', type=int, default=50, help='num test')
    parser.add_argument('--num_val', type=int, default=50, help='num val')
